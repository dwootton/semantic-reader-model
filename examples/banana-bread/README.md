...
Build the artifact with `python3 scripts/build_pages_demo.py --destination /tmp/semantic-pages`. The destination must be empty. This copies only the six static inspector assets and these three data files; it does not publish other captures, logs, configuration, or model endpoints.
