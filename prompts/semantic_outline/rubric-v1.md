# Semantic outline labeler — rubric v1
...
can be compared against it. Version 1 adds explicit nonoverlap and long-form
reading granularity checks based on the saved Python pilot review.

## Required reading-unit boundaries

For long-form documentation, every explanatory paragraph and code/example block
must remain a separately reachable reading unit. API entries and chapter sections
are GROUPS containing those units, not giant leaves. A short heading and its lede
may share one unit. A whole code block may remain one unit when the evidence has
no finer source-backed boundaries; do not invent IDs or split syntax spans into
token-sized stops.

Never label a whole chapter or module container "Overview" and then repeat its
descendant text in later reading units. Reading units must not contain the source
subtrees of other reading units. Choose the smallest real container enclosing the
whole intended unit. Additional refs attach its controls/headings; they are not a
substitute for a missing multi-paragraph container. A heading section with children
must have refs=[]; give its introductory text a separate source-backed child.
Do not discard introductions merely to satisfy group serialization.

Before returning, check ancestor/descendant overlap among leaf source refs, not
just repeated identical IDs. Check that an Overview stop contains only the actual
introductory paragraph(s), not the entire document. Keep each paragraph and code
block reachable without reading a whole section first.

Archive-only or media-suppressed evidence is not an untouched live rendering.
State the supplied evidence limitations in scope. A broken source aria-controls
relationship, an empty-fragment current-page link, or a deferred decorative
search icon does not by itself mean article text is missing. Report these limits
without inventing unseen contents or pointless content-expansion requests.
